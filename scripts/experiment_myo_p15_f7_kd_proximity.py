"""
experiment_myo_p15_f7_kd_proximity.py — F7: 距離変調 endpoint Kd で overshoot 抑制。

F6 (γ-Ia 反射ゲイン変調) は反証された:
  γ_gain=30 まで上げて K_ia 実効値を 14倍にしても progress_ratio が 1.19 → 1.19
  でほぼ動かず。target 付近で muscle_vel が小さく、Ia 信号 (K_ia × max(vel,0))
  が γ で何倍しても効力不足。

F7 はより直接的な機構を試す:
  下行性指令が脊髄の速度フィードバックゲイン (≒ co-contraction 由来の damping)
  を target 接近に応じて増す。endpoint Kd を距離変調する形で実装:
      Kd_eff = Kd_ee * (1 + Kd_proximity_gain * exp(-||reach_err|| / Kd_proximity_scale))

  神経生理: Burdet et al. 2001 Nature, Franklin & Wolpert 2011。
    精密 reach 時、ヒトは拮抗筋同時活性化で関節 impedance (剛性+粘性) を増す。
    ここでは endpoint impedance の damping 成分のみを変調。

設計（事前宣言）:
  env       : myoArmReachRandom-v0 + deterministic_reset
  seeds     : 0..49 から reach_dist<0.85m の最初 20
  Kd_scale  : 0.10 m 固定 (target 半径 10cm 内で大きく effect)
  条件      : 1. neural Kd_g=0  (baseline = F4/F6 neural γ=0 と同等)
              2. neural Kd_g=1
              3. neural Kd_g=3
              4. neural Kd_g=5
              5. neural Kd_g=10
              6. pure_PD Kd_g=0   (F4 比較)
              7. pure_PD Kd_g=5   (神経成分なしでも効くか確認)
  指標      : tip_err_min/final_mm, vel_peak_ratio, straightness, jerk_rms,
              peak_speed, movement_time, direction_error_deg, progress_ratio,
              kd_factor_max/mean
  検定      : 各 Kd_g>0 vs neural Kd_g=0 で Welch's t-test
              主要指標 = progress_ratio (overshoot)、副次 = tip_err_min_mm

期待される結果:
  Kd_g 増加で progress_ratio が 1.19 → 1.0 付近に低下、min_err も改善するはず。
  Kd_g が大きすぎると tip_vel に過剰反応してジャーキー → jerk_rms 増加で検出。

出力:
  results/experiment_myo_p15/f7_kd_proximity.json
"""
...
def run_episode(env, ctrl, seed, max_steps=600) -> dict:
    ...
    for step in range(max_steps):
        ...
        kd_factors.append(info.get("kd_factor", 1.0))
    ...
    return {
        ...
        "kd_factor_max":  float(np.max(kd_factors)) if kd_factors else 1.0,
        "kd_factor_mean": float(np.mean(kd_factors)) if kd_factors else 1.0,
        **km,
    }


def make_cfg(*, Kd_proximity_gain: float, neural: bool) -> MyoArmConfig:
    base = dict(
        ...
        Kd_proximity_gain=Kd_proximity_gain, Kd_proximity_scale=KD_SCALE,
    )
    if neural:
...
def main() -> None:
    print("=== Phase 1-5 F7: 距離変調 endpoint Kd で overshoot 抑制 ===")
    print(f"env: myoArmReachRandom-v0 + deterministic_reset  Kd_scale={KD_SCALE}m  n={N_REACHABLE}")
    ...
    conditions = [
        ("neural Kd_g=0  (baseline)", make_cfg(Kd_proximity_gain=0.0,  neural=True),  True),
        ("neural Kd_g=1",             make_cfg(Kd_proximity_gain=1.0,  neural=True),  True),
        ("neural Kd_g=3",             make_cfg(Kd_proximity_gain=3.0,  neural=True),  True),
        ("neural Kd_g=5",             make_cfg(Kd_proximity_gain=5.0,  neural=True),  True),
        ("neural Kd_g=10",            make_cfg(Kd_proximity_gain=10.0, neural=True),  True),
        ("pure_PD Kd_g=0",            make_cfg(Kd_proximity_gain=0.0,  neural=False), False),
        ("pure_PD Kd_g=5",            make_cfg(Kd_proximity_gain=5.0,  neural=False), False),
    ]
    ...
    for name, cfg, load_cfc in conditions:
        ...
        print(f"    Kd_factor max={s['kd_factor_max']['mean']:5.2f}  mean={s['kd_factor_mean']['mean']:5.2f}  "
    ...
    # ── 統計検定: 各 Kd_g>0 条件 vs neural Kd_g=0 ──
    print("\n=== Welch's t-test: 各 condition vs neural Kd_g=0 (baseline) ===")
    test_results = {}
    base_per_seed = results["neural Kd_g=0  (baseline)"]["per_seed"]
    for key in ["progress_ratio", "tip_err_min_mm", "direction_error_deg", "jerk_rms"]:
        base_vals = [r[key] for r in base_per_seed]
        for name in [n for n, _, _ in conditions if n != "neural Kd_g=0  (baseline)"]:
            ...
            if t["p_value"] is not None:
                ...
                print(f"  {key:<22} {name:<26} t={t['t_stat']:+6.2f}  "
                      f"p={t['p_value']:.4g}{sig:<4}  d={d}")
            else:
                print(f"  {key:<22} {name:<26} (insufficient data)")

    summary = {
        "phase": "1-5 F7",
        "purpose": "距離変調 endpoint Kd で target 付近の damping を増し overshoot 抑制",
        "env": "myoArmReachRandom-v0 + deterministic_reset",
        "dt": DT,
        "Kd_proximity_scale": KD_SCALE,
        "n_seeds": len(seeds),
        "seeds": seeds,
    ...
    out = RESULTS_DIR / "f7_kd_proximity.json"
