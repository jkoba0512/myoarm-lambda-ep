...
\setstretch{1.15}
\hypertarget{decoupling-smoothness-accuracy-and-kinematic-invariance-in-biological-reach-an-ablation-study-of-an-equilibrium-point-controller-in-a-34-muscle-arm-model}{%
\section{Decoupling smoothness, accuracy, and kinematic invariance in
biological reach: an ablation study of an equilibrium-point controller
in a 34-muscle arm
model}\label{decoupling-smoothness-accuracy-and-kinematic-invariance-in-biological-reach-an-ablation-study-of-an-equilibrium-point-controller-in-a-34-muscle-arm-model}}

\textbf{Jun Kobayashi}\(^{1,*}\)

\(^{1}\) Department of Intelligent and Control Systems, Graduate School
of Computer Science and Systems Engineering, Kyushu Institute of
Technology, 680-4 Kawazu, Iizuka, Fukuoka 820-8502, Japan

ORCID: \href{https://orcid.org/0009-0002-6318-2617}{0009-0002-6318-2617}
...
violate the kinematic invariants of human reach: bell-shaped speed
profiles, near-straight paths, and a peak-velocity time at 40--50 \% of
movement duration. For the MyoSuite myoArm (20-DoF, 34 Hill-type
muscles) we implement a biologically motivated controller combining (i)
Feldman's λ-equilibrium-point hypothesis, (ii) a minimum-jerk virtual
trajectory \(\boldsymbol{\lambda}(t)\), (iii) a 200 ms visuomotor
correction, and (iv) γ-compatible spinal reflexes (Ia, Ib, reciprocal
inhibition). Across \(n = 50\) randomised targets the full controller is
practically equivalent to an \textbf{endpoint-PD + spinal} baseline
(Cartesian PD descending command paired with the same spinal reflex
layer; see §2.6) on minimum tip error (Cohen's \(d = +0.03\); paired
Wilcoxon detects only a \(+10.6\) mm residual against a \(\approx 100\)
mm absolute error, well within a pre-defined \(\pm 20\) mm equivalence
margin) while halving peak speed (\(1.78\) vs \(3.90\) m s\(^{-1}\),
\(d = -7.39\), \(p < 10^{-15}\)) and reducing jerk by 40 \%
(\(d = -1.74\)). Only the variant with stretch reflexes brings the
velocity-peak ratio into the canonical human range (\(0.40\)--\(0.50\)).
Straightness stays below the human reference, so we frame the result as
a \emph{partial} reproduction of the bell-shape and smoothness
invariants, not full human-like reach. A factorial ablation (\(n = 20\))
decomposes the contributions: virtual trajectory primarily controls
smoothness, visuomotor feedback primarily controls accuracy, and
reflexes primarily control velocity-peak timing, with two quantifiable
secondary effects reported explicitly. An attempted online cerebellar
correction in joint or λ space did not improve performance, consistent
with --- but not by itself demonstrating --- the cerebellum as a slow
inverse-model learner rather than a within-trial steering controller. We
release a \texttt{deterministic\_reset} patch for a seeding bug in the
MyoSuite reach environments (in the versions tested). The result is
mechanistic rather than task-optimal: it attributes separable kinematic
axes to distinct biological control layers in a 34-muscle arm.

\textbf{Keywords}: equilibrium-point hypothesis · virtual trajectory ·
...
\hypertarget{biological-reach-has-structured-invariants-that-engineering-controllers-tend-to-violate}{%
\subsubsection{1.1 Biological reach has structured invariants that
engineering controllers tend to
violate}\label{biological-reach-has-structured-invariants-that-engineering-controllers-tend-to-violate}}
...
1981; Flash and Hogan 1985); the hand path between two points is nearly
straight in extrinsic space; and the time-derivative of acceleration
(jerk) is minimised in the sense of Hogan (1984). These properties are
...
2022) make it possible to simulate a 34-muscle Hill-type arm and to test
...
\hypertarget{theoretical-framework-ux3bb-ep-virtual-trajectory-and-visuomotor-feedback}{%
\subsubsection{1.2 Theoretical framework: λ-EP, virtual trajectory, and
visuomotor
feedback}\label{theoretical-framework-ux3bb-ep-virtual-trajectory-and-visuomotor-feedback}}

Three mature ideas in motor neuroscience together suggest a complete
controller. \textbf{Feldman's λ-equilibrium-point hypothesis} (Feldman
...
\textbf{virtual-trajectory} extension (Feldman and Levin 1995; Latash
1993) replaces the static \(\boldsymbol{\lambda}_{\rm target}\) with a
...
(Saunders and Knill 2003; Sarlegna and Sainburg 2009) provides a slow (≈
...
cord, \textbf{γ-compatible stretch reflexes} (Prochazka 1989; Loeb
1984), Golgi-tendon load feedback, and reciprocal inhibition shape the
activation locally on a 20--30 ms timescale and prevent co-contraction.
We retain the architectural slot for descending γ-motoneuron modulation
(Prochazka 1989) but disable it in our production controller, since
...
\item
  ...
  spinal reflexes (Fig. 5). The implementation is MIT-licensed and will
  be released with the pre-print, with reproduction commands documented
...
\item
  ...
  (Fig. 3), although both controllers fall short of the human
...
\item
  ...
  their effects (Fig. 4): the virtual trajectory dominates the
...
\hypertarget{paper-organisation}{%
\subsubsection{1.4 Paper organisation}\label{paper-organisation}}
...
\hypertarget{musculoskeletal-environment}{%
\subsubsection{2.1 Musculoskeletal
environment}\label{musculoskeletal-environment}}

We simulate reach in MyoSuite's \texttt{myoArmReachRandom-v0}
environment (Caggiano et al.~2022; tested with MyoSuite 2.12.1, MuJoCo
3.6.0, Gymnasium 1.2.3 on Linux 6.8), which wraps a MuJoCo (Todorov et
al. 2012) model of a human upper limb with 20 kinematic degrees of
...
\hypertarget{ux3bb-equilibrium-point-controller}{%
\subsubsection{2.2 λ-equilibrium-point
controller}\label{ux3bb-equilibrium-point-controller}}

Our controller follows the λ-version of Feldman's equilibrium-point
hypothesis (Feldman 1986; Feldman and Levin 1995). The descending
...
\textbf{Virtual trajectory.} Following Feldman and Levin (1995), the
...
scaled by 1.2× and bounded for short and long reaches respectively. The
activation in Eq. (1) is computed using \(\boldsymbol{\lambda}(t)\) in
place of \(\boldsymbol{\lambda}_{\rm target}\).

\hypertarget{online-visuomotor-feedback}{%
\subsubsection{2.3 Online visuomotor
feedback}\label{online-visuomotor-feedback}}
...
at movement onset and is blind to the actual tip motion. Saunders and
Knill (2003) and Sarlegna and Sainburg (2009) show that human reach
...
\hypertarget{spinal-reflexes}{%
\subsubsection{2.4 Spinal reflexes}\label{spinal-reflexes}}
...
The architecture admits a γ-motoneuron modulation
\(K_{\rm Ia}\to K_{\rm Ia}\,(1+\gamma\exp(-\|\mathbf{e}\|/\sigma_{\gamma}))\)
on target approach (Prochazka 1989), but \(\gamma = 0\) in all
configurations reported below (γ-on configurations were tested in pilot
...
\hypertarget{cerebellar-forward-model-negative-result-branch}{%
\subsubsection{2.5 Cerebellar forward model (negative-result
branch)}\label{cerebellar-forward-model-negative-result-branch}}
...
interpretations of cerebellar output (Kawato 1999; Wolpert et al. 1998;
Ito 2008). The \emph{joint-space} pathway maps the prediction error
...
The \emph{λ-space} pathway, more consistent with the EP framework, maps
...
on prediction-error spikes (Ito 2008); the firing-rate calibration and
...
(joint pathway) and \(K_{\rm cereb}^{\lambda} = 0.5\) (λ pathway) on top
of the full λ-EP controller with reflexes; both gains were the best
among a small sweep (\(K_{\rm cereb}\in\{0.05,0.1,0.2,0.5\}\),
\(K_{\rm cereb}^{\lambda}\in\{0.1,0.5,1.0\}\)).

\hypertarget{endpoint-pd-baseline}{%
\subsubsection{2.6 Endpoint-PD baseline}\label{endpoint-pd-baseline}}
...
layer (Ia, Ib, RI; §2.4) with the full λ-EP controller, so that the
headline comparison contrasts the \emph{descending command} (Cartesian
endpoint PD vs λ-EP with virtual trajectory and visuomotor feedback)
...
configuration, whereas the full λ-EP controller has no cerebellar
...
error, exactly zero on peak speed and vpr; complete metric × test table
...
\subsubsection{2.7 Evaluation protocol}\label{evaluation-protocol}}
...
reproducibility. For the factorial ablation (Fig. 4) we use \(n = 20\)
...
distribution (Figs 1--3) we use the full \(n = 50\).
...
reference: 0.40--0.50; Morasso 1981; Flash and Hogan 1985);
...
\def\labelenumi{\arabic{enumi}.}
\item
  \emph{Headline comparison (Table 1, Figs 1--3, §3.1).} The reference
  is the \textbf{endpoint-PD + spinal baseline} described in §2.6
  (M1-level contrast, with the spinal layer held common).
\item
  \emph{Factorial ablation (Fig. 4, §3.2).} The reference is the
  \textbf{pure λ + visuomotor controller} (no reflexes, no cerebellar
  correction).
\end{enumerate}
...
primary effect-size measure throughout. For Fig. 4, \(d\) is signed so
...
paired differences and run a two one-sided \(t\)-test (TOST, Lakens
2017):
...
\item
  \emph{Direction error}: \(\pm 5°\), chosen as one-fifth of the typical
...
\textbf{Compute and reproducibility.} Each condition × seed run takes
...
trained CfC weights will be released with the pre-print, with
reproduction commands documented in the repository README.
...
\hypertarget{the-full-controller-matches-pd-accuracy-with-halved-peak-speed-and-human-range-peak-timing}{%
\subsubsection{3.1 The full controller matches PD accuracy with halved
peak speed and human-range peak
timing}\label{the-full-controller-matches-pd-accuracy-with-halved-peak-speed-and-human-range-peak-timing}}
...
(\(\pm 5°\); \(p_{\rm TOST} = 1.1 \times 10^{-8}\)). The seed-paired
Wilcoxon test nevertheless detects small residual shifts on minimum tip
error (\(p = 0.047\), median \(+10.6\) mm, \(d = +0.03\); Table 1, Fig.
1d) and on direction error (\(p = 0.039\), median \(+2.0°\),
...
human reaching without a final corrective movement (Sarlegna and
Sainburg 2009); we report all metrics on the full \(n = 50\) regardless
of solve status.
...
\(p < 10^{-25}\); Fig. 1a) and reduces root-mean-square jerk by ≈ 40 \%
...
\(0.221 \pm 0.170\) for the pure λ + visuomotor controller without
...
human band (Flash and Hogan 1985). Speed-profile skewness drops from
...
oscillate around the residual offset; the λ-EP paths approach the target
on a smoother arc and the distance-to-target curve is monotone over the
movement window (Fig. 3c).
...
s\(^{-1}\) followed by oscillatory residuals (Fig. 2a, gray), the pure λ
+ visuomotor controller (Fig. 2a, light blue) shows a broad
...
(Fig. 2b). The per-seed vpr distribution (Fig. 2c) is centred inside the
...
\hypertarget{each-component-contributes-preferentially-to-one-task-axis-with-structured-cross-talk}{%
\subsubsection{3.2 Each component contributes preferentially to one task
axis, with structured
cross-talk}\label{each-component-contributes-preferentially-to-one-task-axis-with-structured-cross-talk}}
...
components from the full λ-EP controller and re-evaluated on \(n = 20\)
targets (Fig. 4). This lower-powered ablation is used for
...
comparison of §3.1. We treat the \emph{pure λ + visuomotor} controller
...
initial-transient bump visible in Fig. 2a.

The full picture (Fig. 4b) is a near-block-diagonal pattern (virtual
...
\hypertarget{online-cerebellar-correction-does-not-improve-performance}{%
\subsubsection{3.3 Online cerebellar correction does not improve
performance}\label{online-cerebellar-correction-does-not-improve-performance}}

Two cerebellar correction pathways were evaluated on top of the full
λ-EP+reflex controller on \(n = 50\) seeds: (i) a \emph{joint-space}
pathway (CfC \(\to\) torque correction \(\to\)
\(\mathbf{J}_{\rm act}^{+}\) projection) at \(K_{\rm cereb} = 0.2\), and
(ii) a \emph{λ-space} pathway (CfC \(\to\)
...
proprioceptive feedback. The λ-space correction degrades the bell shape
...
\(\{0.1, 0.5, 1.0\}\) for the λ pathway and found no setting that
simultaneously improves accuracy, smoothness, and bell shape; the full
sweep table will be released with the pre-print.
...
\emph{within-trial} steering (Wolpert et al.~1998; Ito 2008; discussion
in §4).

\hypertarget{validation-of-the-seed-reproducibility-patch}{%
\subsubsection{3.4 Validation of the seed-reproducibility
patch}\label{validation-of-the-seed-reproducibility-patch}}
...
\hypertarget{bell-shape-and-smoothness-invariants-emerge-from-biologically-motivated-control}{%
\subsubsection{4.1 Bell-shape and smoothness invariants emerge from
biologically motivated
control}\label{bell-shape-and-smoothness-invariants-emerge-from-biologically-motivated-control}}
...
and γ-compatible spinal reflexes --- is practically equivalent to an
...
they can emerge in this architecture when the λ-equilibrium-point
...
\hypertarget{why-these-specific-component-to-axis-assignments}{%
\subsubsection{4.2 Why these specific component-to-axis
assignments}\label{why-these-specific-component-to-axis-assignments}}
...
The visuomotor loop is a slow estimator of the \emph{target} in λ space.
...
\hypertarget{cerebellum-as-a-slow-learner-not-a-within-trial-steerer}{%
\subsubsection{4.3 Cerebellum as a slow learner, not a within-trial
steerer}\label{cerebellum-as-a-slow-learner-not-a-within-trial-steerer}}
...
inverse-model \emph{learning} (Wolpert et al. 1998; Kawato 1999; Ito
2008) rather than within-trial steering.
...
reach without a final corrective sub-movement (Sarlegna and Sainburg
2009). We deliberately report metric distributions rather than solve
...
reaches (Morasso 1981). Inspection of the trajectories shows that this
is dominated by \emph{post-target} path wandering during the late phase
of the episode (the reach window is 12 s but the movement window is ≈ 2
...
closer to the human range. A task-space virtual trajectory (Morasso
1981; Flash and Hogan 1985) computed in end-effector coordinates rather
than in λ space did improve approach straightness in pilot experiments
...
(Schumacher et al.~2023). Our argument is not that the biological
...
at the onset of the +reflex speed profile (Fig. 2a) is a real artefact
...
\subsubsection{4.5 Implications and future
...
λ-equilibrium-point hypothesis with a minimum-jerk virtual trajectory, a
200 ms visuomotor correction loop, and a stack of γ-compatible spinal
...
report explicitly. An online cerebellar correction in either joint or λ
...
\textbf{Fig. 1 --- Ablation of biological reach control mechanisms.}
\emph{Minimum-tip-error distributions overlap across all controllers
(panel d), but only the biologically-grounded λ-EP variants partially
reproduce the peak-timing and smoothness invariants of human reach.}
\textbf{(a)} Peak tip speed. Endpoint-PD + spinal (gray) overshoots at
\(\approx 3.9\) m s\(^{-1}\), whereas the static λ-trajectory baseline
(light blue) and the λ + visuomotor controller with reflexes (dark blue)
remain within \(\approx 1.5\)--\(1.8\) m s\(^{-1}\), matching reported
human values for \(\sim 50\) cm reaches (Morasso 1981). \textbf{(b)}
Root-mean-square jerk. Reflexes do not increase smoothness costs.
\textbf{(c)} Velocity-peak ratio (vpr): the time of peak speed
normalised by movement duration. Only the +reflex controller falls
inside the human band (\(0.40\)--\(0.50\), green). \textbf{(d)} Minimum
tip-error. The endpoint-PD + spinal, pure λ + visuomotor, and +reflex
headline conditions cluster near \(\approx 100\) mm and remain within
the pre-defined \(\pm 20\) mm practical equivalence margin (§2.7, §3.1);
the no-visuomotor λ-trajectory condition reaches less accurately
(\(\approx 148\) mm), consistent with §3.2 where visuomotor feedback is
identified as the primary accuracy component. Bars show mean ± SEM
(\(n = 50\) randomised seeds per condition). Significance is reported
relative to the endpoint-PD + spinal baseline (§2.6) by the seed-paired
Wilcoxon signed-rank test (*\(p<0.05\), **\(p<0.01\), ***\(p<0.001\);
n.s. \(=\) not significant), in line with the paired seed design
described in §2.7.
...
\textbf{Fig. 2 --- Velocity profiles.} \emph{Adding spinal reflexes to
the λ-EP controller transforms a multi-peak speed profile into a single
dominant profile whose peak timing falls inside the human reference
band.} \textbf{(a)} Tip-speed profiles for a single representative seed
(seed 27, \(n = 1\)). The endpoint-PD + spinal baseline (gray) produces
an initial ballistic spike followed by oscillatory residuals. The pure λ
+ visuomotor controller (light blue) shows two prolonged peaks. Adding
stretch reflexes (dark blue) yields a single dominant peak near \(1.7\)
m s\(^{-1}\) with peak timing inside the human band; the profile retains
a small initial transient and a residual right- skewness (see §3.1).
\textbf{(b)} Time-warped mean speed across \(n = 20\) seeds (the
ablation seed list of §3.2; movement window defined by speed \(> 0.02\)
...
inside this band. \textbf{(c)} Per-seed velocity-peak ratio across the
headline \(n = 50\) seeds of §3.1. Black bars \(=\) mean. Green band
\(=\) human reference range (Morasso 1981; Flash and Hogan 1985). The
change of \(n\) between panels (b) and (c) is intentional: panel (b)
shares the \(n = 20\) seed list with the ablation analysis (Fig. 4), and
panel (c) reports the headline \(n = 50\) distribution.

\begin{center}
\includegraphics[width=\textwidth]{../figures/fig3_trajectories.pdf}
\end{center}

\textbf{Fig. 3 --- End-effector trajectories.} \emph{The λ-EP controller
produces straighter and more direct end-effector paths than the
engineering baseline.} \textbf{(a)} 3-D reach paths for a representative
seed (seed 27). Black circle \(=\) start, red star \(=\) target. The
straightness ratio \(S\) (straight-line distance / path length, higher
\(=\) straighter) is shown in each legend entry. \textbf{(b)} Sagittal
projection (y--z plane) of the same trajectories, illustrating the
qualitative difference between the early ballistic overshoot of
endpoint-PD and the smoother arc of the λ-EP controllers. \textbf{(c)}
Distance-to-target time courses. The λ-EP controllers approach the
target monotonically; endpoint-PD oscillates around the residual
\(\approx 100\) mm offset throughout the 2 s movement window.

\begin{center}
\includegraphics[width=\textwidth]{../figures/fig4_orthogonal.pdf}
\end{center}

\textbf{Fig. 4 --- Component contributions of the three controller
layers.} \emph{Each controller component contributes preferentially to
one task dimension (smoothness, accuracy, or velocity-peak timing), with
two practically-sized off-diagonal effects.} \textbf{Reference
configuration is the pure λ + visuomotor controller (no reflexes, no
cerebellar correction), not the endpoint-PD + spinal baseline of §3.1;
signed Cohen's \(d\) is reported with the convention positive =
improvement (lower for error / peak speed / jerk; higher for vpr).}
...
separately in §3.3 and is not part of this ablation) --- project onto
...
\(d\) based on the primary metric for each axis: peak speed, minimum tip
error, and velocity-peak ratio respectively); red arrows mark
degradations of equivalent magnitude; thin dashed gray arrows mark weak
effects (\(|d| \le 0.5\)). \textbf{(b)} Signed Cohen's \(d\) for each
component × metric pair (\(n = 20\) seeds per condition; data from the
F13 ablation). Positive values (blue) indicate that adding the component
pushes the metric in the behaviourally desirable direction (lower for
error / speed / jerk; higher for vpr). Asterisks denote the seed-paired
Wilcoxon signed-rank test (*\(p<0.05\), **\(p<0.01\), ***\(p<0.001\));
Cohen's \(d\) is shown as the distribution-level effect size. The
block-diagonal pattern demonstrates that the three components contribute
non-redundantly, with two practically-sized off-diagonal terms ---
virtual trajectory has a secondary positive effect on bell shape
(\(d = +1.07\)**) and reflexes have a moderate negative effect on
peak-speed control (\(d = -1.55\)***) --- and two small but
paired-significant accuracy costs of reflexes (\(d = -0.17\)* on minimum
tip error, \(d = -0.23\)* on direction error).

\begin{center}
\includegraphics[width=\textwidth]{../figures/fig5_architecture.pdf}
\end{center}

\textbf{Fig. 5 --- Controller architecture.} \emph{Block diagram of the
neuromusculoskeletal reach controller, organised by anatomical level.
Cortical, cerebellar and spinal blocks are kept distinct; the figure is
a functional schematic, not a strict neuroanatomical mapping.} A target
position \(\mathbf{x}_{\rm target}\) provided by the visual system (top)
drives an inverse-kinematics solver at the \textbf{cortical/planning}
level that returns the equilibrium muscle thresholds
...
\(s(\tau) = 10\tau^{3}-15\tau^{4}+6\tau^{5}\) (Flash and Hogan 1985). A
separate \textbf{parietal--motor visuomotor block} updates
\(\boldsymbol{\lambda}_{\rm target}\) from the observed tip position
every 200 ms, and an \emph{optional} \textbf{cerebellar branch} (dashed
in the figure) adds a predictive correction
\(\Delta\boldsymbol{\lambda}_{\rm cereb}\) from a CfC forward-model
prediction error (this branch is the negative-result configuration of
§2.5 and §3.3 --- disabled in the headline controller). The resulting
effective threshold \(\boldsymbol{\lambda}_{\rm eff}\) projects to the
\textbf{spinal} α-motoneuron pool, where a base activation
...
(γ-MN-modulation slot retained but disabled, see §2.4), a Golgi- tendon
component \(\Delta\mathbf{a}_{\rm Ib}\), and reciprocal inhibition
\(\Delta\mathbf{a}_{\rm RI}\). The summed activation
\(\mathbf{a}_{\rm total}\) drives the MyoSuite myoArm muscle plant (34
muscles, 20-DoF arm), whose output state
\((\mathbf{q}, \dot{\mathbf{q}}, \mathbf{x}_{\rm tip})\) feeds back to
the spinal pool with a 20 ms proprioceptive delay (left loop) and to the
visuomotor block with a 100--200 ms visual delay (right loop). Component
colours match those used in Figs 1--4.
...
achievement of final head position. \emph{Journal of Neurophysiology}
39, 435--444. doi: 10.1152/jn.1976.39.2.435.

Burdet, E., Osu, R., Franklin, D. W., Milner, T. E., Kawato, M. (2001).
The central nervous system stabilizes unstable dynamics by learning
optimal impedance. \emph{Nature} 414, 446--449. doi: 10.1038/35106566.

Caggiano, V., Wang, H., Durandau, G., Sartori, M., Kumar, V. (2022).
MyoSuite: a contact-rich simulation suite for musculoskeletal motor
control. In \emph{Proceedings of the 4th Annual Learning for Dynamics
and Control Conference (L4DC)}. Proceedings of Machine Learning Research
168, 492--507. arXiv:2205.13600.
\url{https://proceedings.mlr.press/v168/caggiano22a.html}.

Feldman, A. G. (1966). Functional tuning of the nervous system with
control of movement or maintenance of a steady posture. II. Controllable
parameters of the muscle. \emph{Biophysics} 11, 565--578 (English
translation from \emph{Biofizika} 11(3): 498--508).

Feldman, A. G. (1986). Once more on the equilibrium-point hypothesis (λ
model) for motor control. \emph{Journal of Motor Behavior} 18(1),
17--54. doi: 10.1080/00222895.1986.10735369.

Feldman, A. G., Levin, M. F. (1995). The origin and use of positional
frames of reference in motor control. \emph{Behavioral and Brain
Sciences} 18(4), 723--744. doi: 10.1017/S0140525X0004070X.

Flash, T., Hogan, N. (1985). The coordination of arm movements: an
experimentally confirmed mathematical model. \emph{Journal of
Neuroscience} 5(7), 1688--1703. doi: 10.1523/jneurosci.05-07-01688.1985.

Hasani, R., Lechner, M., Amini, A., Liebenwein, L., Ray, A.,
Tschaikowski, M., Teschl, G., Rus, D. (2022). Closed-form
continuous-time neural networks. \emph{Nature Machine Intelligence} 4,
992--1003. doi: 10.1038/s42256-022-00556-7.

Hogan, N. (1984). An organizing principle for a class of voluntary
movements. \emph{Journal of Neuroscience} 4(11), 2745--2754. doi:
10.1523/jneurosci.04-11-02745.1984.

Ito, M. (2008). Control of mental activities by internal models in the
cerebellum. \emph{Nature Reviews Neuroscience} 9, 304--313. doi:
10.1038/nrn2332.

Kawato, M. (1999). Internal models for motor control and trajectory
planning. \emph{Current Opinion in Neurobiology} 9, 718--727. doi:
...
Latash, M. L. (1993). \emph{Control of human movement}. Human Kinetics.

Loeb, G. E. (1984). The control and responses of mammalian muscle
spindles during normally executed motor tasks. \emph{Exercise and Sport
Sciences Reviews} 12, 157--204. doi: 10.1249/00003677-198401000-00008.

Morasso, P. (1981). Spatial control of arm movements. \emph{Experimental
Brain Research} 42, 223--227. doi: 10.1007/bf00236911.

Prochazka, A. (1989). Sensorimotor gain control: a basic strategy of
motor systems? \emph{Progress in Neurobiology} 33, 281--307. doi:
10.1016/0301-0082(89)90004-x.

Sarlegna, F. R., Sainburg, R. L. (2009). The roles of vision and
proprioception in the planning of reaching movements. In \emph{Progress
...
\emph{Experimental Brain Research} 152, 341--352. doi:
10.1007/s00221-003-1525-2.

Schumacher, P., Häufle, D. F. B., Büchler, D., Schmitt, S., Martius, G.
(2023). DEP-RL: embodied exploration for reinforcement learning in
overactuated and musculoskeletal systems. In \emph{Proceedings of the
11th International Conference on Learning Representations (ICLR)}.
arXiv:2206.00484.

Todorov, E., Erez, T., Tassa, Y. (2012). MuJoCo: a physics engine for
model-based control. In \emph{IEEE/RSJ International Conference on
Intelligent Robots and Systems} (IROS), 5026--5033. doi:
10.1109/iros.2012.6386109.

Wolpert, D. M., Miall, R. C., Kawato, M. (1998). Internal models in the
cerebellum. \emph{Trends in Cognitive Sciences} 2(9), 338--347. doi:
